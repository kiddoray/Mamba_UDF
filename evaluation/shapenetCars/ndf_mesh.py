...
def rotate_mesh_around_y(vertices, angle):
    ...
    vertices_res = rotated_vertices.transpose()
...
def evaluate_mesh(gt_path, es_path, n_points=100000, normalize=False):
    ...
    es_mesh.vertices = rotate_mesh_around_y(es_mesh.vertices, 90) 
    gt_scale = 1.
   ...
   # o3d.io.write_point_cloud("/home/feiben2/UDF-DCD/{gt_path}.ply", pcd)

    total_size = (gt_mesh.bounds[1] - gt_mesh.bounds[0]).max()
    print('es bounds:', es_mesh.bounds)
    es_size = (es_mesh.bounds[1] - es_mesh.bounds[0]).max()
    print('es size:', es_size)
    print('gt bounds:', gt_mesh.bounds)
    print('gt size:', total_size)
    centers = (gt_mesh.bounds[1] + gt_mesh.bounds[0]) / 2
    es_mesh.apply_scale(total_size/es_size)
    # es_mesh.apply_scale(total_size)
    es_mesh.apply_translation(centers)
    print(es_mesh.bounds)

    thresholds= [0.005, 0.01]

    result=evaluator.eval_mesh(es_mesh, gt_pointcloud, gt_normals, thresholds=thresholds)

    dcdl2, _, _ = dcd.calc_dcd(es_mesh, gt_pointcloud)
    ...
    # print("dcd:%.3f"%(dcdl2))
    print('dcd:', dcdl2.mean())
    print("normals:%.3f"%(result['normals']))
...
if __name__ == '__main__':
    ...
    pred_file = os.path.join('/home/feiben2/gifs/experiments/shapenet_cars/evaluation/generation/02958343', args.dir, 'gifs_result.obj')
    # pred_file = os.path.join('/home/feiben2/ndf/experiments/shapenet_cars_pretrained/evaluation/generation/02958343', args.dir, 'ndf.obj')
    # pred_file = os.path.join(conf.get_string('general.base_exp_dir'), args.dir, 'mesh', 'ndf.obj')
