...
from models.mamba_udf import MambaNetwork
from models.knn_mamba import KNNMambaNetwork
import argparse
...
import sys
sys.path.append(os.path.join('/mnt/data/digital_content_aigc/feiben/CAP-UDF', "ChamferDistancePytorch"))
...
class Runner:
    def __init__(self, args, conf_path):
        ...
        # self.udf_network = MambaNetwork(**self.conf['model.udf_network']).to(self.device)

        # # knn mamba
        self.udf_network = KNNMambaNetwork(**self.conf['model.udf_network']).to(self.device)
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(self.iter_step, self.step2_maxiter)):
            ...
            # knn settings
            gradients_sample = self.udf_network.gradient(samples, points).squeeze() # 5000x3
            udf_sample = self.udf_network.udf(samples, points)                      # 5000x1
            ...
            # writer.add_scalar("1005 knn loss",loss, self.iter_step)
            ...
            # if self.iter_step < 40000:
            #     loss = loss_cd 
            # elif self.iter_step < 60000:
            #     loss = loss_dcd.mean()
            # else:
            #     loss = loss_dcd.mean() + loss_view
            #    # loss = loss_view.requires_grad_()
    ...
    def extract_mesh(self, resolution=64, threshold=0.0, point_gt=None, iter_step=0, logger=None):
        ...
        mesh = extract_geometry(bound_min, bound_max, resolution=resolution, threshold=threshold, \
                ...
                query_func=lambda pts: self.udf_network.udf(pts, point_gt), grad_func=lambda pts: self.udf_network.gradient(pts, point_gt))
    ...
    def gen_extra_pointcloud(self, iter_step, low_range):
        ...
        while gen_nums < num_points:
            ...
            samples.requires_grad = True
            gradients_sample = self.udf_network.gradient(samples, point_gt).squeeze() # 5000x3
            udf_sample = self.udf_network.udf(samples, point_gt)                      # 5000x1
            grad_norm = F.normalize(gradients_sample, dim=1)                # 5000x3
            sample_moved = samples - grad_norm * udf_sample                 # 5000x3
